...
typedef struct buffer_s {
  ...
  /** Last embedded FIR seq applied to channel_model (OAI_SIONNA_FIR_EMBED); -1 = never */
  long long sionna_fir_applied_seq;
} buffer_t;
...
#ifdef OAI_PYTHON_EMBED
  // If enabled, apply Sionna-provided flat-fading MIMO matrix directly (no chanmod convolution)
  int sionna_apply;
  double sionna_scale;
  int sionna_diag_only;
  int sionna_shared_mode;
  char sionna_shared_file[4096];
  long long sionna_last_seq;
  pthread_mutex_t sionna_mu;
  pthread_t sionna_thread;
  volatile int sionna_running;
  double *sionna_h_flat; // [re00, im00, re01, im01, ...] row-major by (rx,tx)
  int sionna_h_len;
  int sionna_rx_ant;
  int sionna_tx_ant;
  /* Sionna FIR taps -> chanmod::ch (same math as oai_sionna_fir_taps.py, in-process) */
  int sionna_fir_embed;
  long long sionna_fir_seq;
  int sionna_fir_L;
  int sionna_fir_nr;
  int sionna_fir_nt;
  double *sionna_fir_reim;
  size_t sionna_fir_reim_cap;
  pthread_mutex_t sionna_fir_mu;
  pthread_t sionna_fir_thread;
  volatile int sionna_fir_running;
#endif
} rfsimulator_state_t;
...
#ifdef OAI_PYTHON_EMBED
/**
 * Directory that contains `sionna-main/` (oai_sionna_fir_embed.py, oai_sionna_fir_embed_siso.py,
 * oai_sionna_fir_embed_mimo2x2.py, …).
 * When running from cmake_targets/ran_build/build, cwd is not the repo root — set OAI_SIONNA_REPO_ROOT.
 */
static void rfsim_get_sionna_repo_base(char *out, size_t outsz)
{
  const char *env = getenv("OAI_SIONNA_REPO_ROOT");
  if (env != NULL && env[0] != '\0') {
    snprintf(out, outsz, "%s", env);
    return;
  }
  if (getcwd(out, outsz) != NULL)
    return;
  snprintf(out, outsz, ".");
}

/* Python producer / oai_sionna_fir_taps: OAI_SIONNA_LOG_SEQ_MOD (default 100). */
static int rfsim_sionna_log_seq_mod(void)
{
  const char *ev = getenv("OAI_SIONNA_LOG_SEQ_MOD");
  if (ev == NULL || ev[0] == '\0')
    ev = getenv("OAI_SIONNA_LOG_EVERY_UPDATES"); /* backward compatibility */
  int n = (ev && ev[0] != '\0') ? atoi(ev) : 100;
  if (n < 1)
    n = 1;
  return n;
}
...
/*
 * gNB/UE rfsimulator [SIONNA][RFSIM] "Updated H" print rate only (FIR + flat embed).
 * Default: 10 * OAI_SIONNA_LOG_SEQ_MOD so HW logs are 1/10 as frequent as the producer.
 * Override: OAI_SIONNA_RFSIM_LOG_SEQ_MOD (absolute modulus on FIR seq / flat update count).
 */
static int rfsim_sionna_hw_channel_log_seq_mod(void)
{
  const char *ev = getenv("OAI_SIONNA_RFSIM_LOG_SEQ_MOD");
  if (ev != NULL && ev[0] != '\0') {
    int n = atoi(ev);
    return (n < 1) ? 1 : n;
  }
  int base = rfsim_sionna_log_seq_mod();
  long long v = (long long)base * 10LL;
  if (v > (long long)INT_MAX)
    return INT_MAX;
  return (int)v;
}

static int load_shared_h_snapshot(rfsimulator_state_t *t)
{
  FILE *f = fopen(t->sionna_shared_file, "r");
  if (!f)
    return -1;

  long long seq = -1;
  int rx = 0, tx = 0;
  if (fscanf(f, "%lld %d %d", &seq, &rx, &tx) != 3) {
    fclose(f);
    return -2;
  }

  if (rx != t->sionna_rx_ant || tx != t->sionna_tx_ant) {
    fclose(f);
    return -3;
  }
  ...
  pthread_mutex_lock(&t->sionna_mu);
  if (t->sionna_h_len != n) {
    free(t->sionna_h_flat);
    t->sionna_h_flat = calloc((size_t)n, sizeof(double));
    t->sionna_h_len = n;
  }
  if (t->sionna_h_flat)
    memcpy(t->sionna_h_flat, tmp, (size_t)n * sizeof(double));
  t->sionna_last_seq = seq;
  pthread_mutex_unlock(&t->sionna_mu);
  free(tmp);
  return 0;
}

static void *sionna_embed_loop_rfsim(void *arg)
{
  rfsimulator_state_t *t = (rfsimulator_state_t *)arg;
  const char *upd_env = getenv("OAI_SIONNA_UPDATE_US");
  int update_us = upd_env ? atoi(upd_env) : 5000000; // default 5s
  if (update_us < 1000)
    update_us = 1000;

  if (t->sionna_shared_mode) {
    while (t->sionna_running) {
      const int rc = load_shared_h_snapshot(t);
      if (rc == 0) {
        double re00 = 0.0, im00 = 0.0;
        long long seq = -1;
        pthread_mutex_lock(&t->sionna_mu);
        if (t->sionna_h_flat && t->sionna_h_len >= 2) {
          re00 = t->sionna_h_flat[0];
          im00 = t->sionna_h_flat[1];
          seq = t->sionna_last_seq;
        }
        pthread_mutex_unlock(&t->sionna_mu);
        LOG_I(HW,
              "[SIONNA][RFSIM] Shared H seq=%lld (%dx%d), H[0,0]=%f + j%f (scale=%f)\n",
              seq,
              t->sionna_rx_ant,
              t->sionna_tx_ant,
              re00,
  ...
  char repo_base[4096] = {0};
  rfsim_get_sionna_repo_base(repo_base, sizeof(repo_base));

  char cmd[8192];
  /* sionna-main/: oai_channel_embed.py — sionna-main/src/: Sionna package */
  snprintf(cmd,
           sizeof(cmd),
           "import sys\n"
           "root = r'%s/sionna-main'\n"
           "src = r'%s/sionna-main/src'\n"
           "for p in (src, root):\n"
  ...
  PyObject *mod = PyImport_ImportModule("oai_channel_embed");
  if (!mod) {
    PyErr_Print();
    LOG_E(HW, "[SIONNA][RFSIM] Failed to import oai_channel_embed\n");
  ...
  if (init_fn && PyCallable_Check(init_fn)) {
    ...
    PyDict_SetItemString(kwargs, "num_rx", PyLong_FromLong(1));
    PyDict_SetItemString(kwargs, "num_tx", PyLong_FromLong(1));
    PyDict_SetItemString(kwargs, "num_rx_ant", PyLong_FromLong(t->sionna_rx_ant));
    PyDict_SetItemString(kwargs, "num_tx_ant", PyLong_FromLong(t->sionna_tx_ant));
    PyObject *ret = PyObject_Call(init_fn, PyTuple_New(0), kwargs);
    Py_DECREF(kwargs);
    if (!ret) {
      PyErr_Print();
      LOG_E(HW, "[SIONNA][RFSIM] init() call failed\n");
    } else {
      Py_DECREF(ret);
    }
  }
  ...
  PyObject *get_fn = PyObject_GetAttrString(mod, "get_h_flat");
  if (!get_fn || !PyCallable_Check(get_fn)) {
    LOG_E(HW, "[SIONNA][RFSIM] get_h_flat not callable\n");
    Py_XDECREF(get_fn);
    Py_DECREF(mod);
    PyGILState_Release(gstate);
    return NULL;
  ...
  const int log_every = rfsim_sionna_hw_channel_log_seq_mod();
  unsigned long long upd_count = 0;
  while (t->sionna_running) {
    PyObject *lst = PyObject_CallObject(get_fn, NULL);
    if (!lst) {
      PyErr_Print();
      usleep(200 * 1000);
      continue;
    ...
    if (PyList_Check(lst)) {
      ...
      pthread_mutex_lock(&t->sionna_mu);
      if (t->sionna_h_len != (int)n) {
        free(t->sionna_h_flat);
        t->sionna_h_flat = calloc((size_t)n, sizeof(double));
        t->sionna_h_len = (int)n;
      }
      for (Py_ssize_t i = 0; i < n; i++) {
        PyObject *item = PyList_GetItem(lst, i); // borrowed
        if (item)
          t->sionna_h_flat[i] = PyFloat_AsDouble(item);
      }
      pthread_mutex_unlock(&t->sionna_mu);
      ...
      pthread_mutex_unlock(&t->sionna_mu);
      upd_count++;
      if ((upd_count % (unsigned long long)log_every) == 0) {
        LOG_I(HW, "[SIONNA][RFSIM] Updated H (%dx%d), H[0,0]=%f + j%f (scale=%f)\n",
              t->sionna_rx_ant, t->sionna_tx_ant, re00, im00, t->sionna_scale);
      }
    }
    Py_DECREF(lst);
    usleep(update_us);
...
static void *sionna_fir_embed_loop_rfsim(void *arg)
{
  rfsimulator_state_t *t = (rfsimulator_state_t *)arg;
  /* Same cadence as flat H: OAI_SIONNA_FIR_UPDATE_US, else OAI_SIONNA_UPDATE_US, else 5 s */
  const char *fir_upd = getenv("OAI_SIONNA_FIR_UPDATE_US");
  const char *h_upd = getenv("OAI_SIONNA_UPDATE_US");
  int sleep_us;
  if (fir_upd != NULL && fir_upd[0] != '\0')
    sleep_us = atoi(fir_upd);
  else if (h_upd != NULL && h_upd[0] != '\0')
    sleep_us = atoi(h_upd);
  else
    sleep_us = 5000000;
  if (sleep_us < 1000)
  ...
  if (!Py_IsInitialized()) {
    LOG_E(HW, "[SIONNA][RFSIM] Python not initialized on main thread; abort FIR worker\n");
    return NULL;
  }
  ...
  const char *fir_mod = getenv("OAI_SIONNA_FIR_MODULE");
  if (fir_mod == NULL || fir_mod[0] == '\0')
    fir_mod = "oai_sionna_fir_embed";
  LOG_I(HW, "[SIONNA][RFSIM] FIR module: %s\n", fir_mod);

  PyObject *mod = PyImport_ImportModule(fir_mod);
  if (!mod) {
    PyErr_Print();
    LOG_E(HW, "[SIONNA][RFSIM] import %s failed\n", fir_mod);
    PyGILState_Release(gstate);
    return NULL;
  }
  ...
  PyObject *init_fn = PyObject_GetAttrString(mod, "init");
  if (init_fn && PyCallable_Check(init_fn)) {
    int nrx = t->rx_num_channels > 0 ? t->rx_num_channels : 1;
    int ntx = t->tx_num_channels > 0 ? t->tx_num_channels : 1;
    const char *rx_env = getenv("OAI_SIONNA_RX_ANT");
    const char *tx_env = getenv("OAI_SIONNA_TX_ANT");
    if (rx_env)
      nrx = atoi(rx_env);
    if (tx_env)
      ntx = atoi(tx_env);
    ...
    if (ntx <= 0)
      ntx = 1;
    PyObject *kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "num_rx_ant", PyLong_FromLong(nrx));
    PyDict_SetItemString(kwargs, "num_tx_ant", PyLong_FromLong(ntx));
    PyObject *args_empty = PyTuple_New(0);
    PyObject *ret = PyObject_Call(init_fn, args_empty, kwargs);
    Py_DECREF(args_empty);
    Py_DECREF(kwargs);
    if (!ret) {
      PyErr_Print();
      LOG_E(HW, "[SIONNA][RFSIM] %s init() failed\n", fir_mod);
    } else {
      Py_DECREF(ret);
    }
  }
  ...
  PyObject *get_fn = PyObject_GetAttrString(mod, "get_fir_snapshot");
  if (!get_fn || !PyCallable_Check(get_fn)) {
    LOG_E(HW, "[SIONNA][RFSIM] get_fir_snapshot not callable\n");
    Py_XDECREF(get_fn);
    Py_DECREF(mod);
    PyGILState_Release(gstate);
    return NULL;
  ...
  const int log_seq_mod = rfsim_sionna_hw_channel_log_seq_mod();
  long long last_snap_seq_seen = -2;
  while (t->sionna_fir_running) {
    PyObject *lst = PyObject_CallObject(get_fn, NULL);
    ...
    Py_ssize_t n = PyList_Size(lst);
    if (n < 5) {
      LOG_E(HW, "[SIONNA][RFSIM] FIR snapshot list too short (%zd)\n", n);
      Py_DECREF(lst);
      usleep(200 * 1000);
      continue;
    }

    long long seq = PyLong_AsLongLong(PyList_GetItem(lst, 0));
    if (PyErr_Occurred()) {
      PyErr_Clear();
      Py_DECREF(lst);
      continue;
    }
    if (seq == last_snap_seq_seen) {
      Py_DECREF(lst);
    ...
    last_snap_seq_seen = seq;

    int L = (int)PyLong_AsLong(PyList_GetItem(lst, 1));
    int nr = (int)PyLong_AsLong(PyList_GetItem(lst, 2));
    int nt = (int)PyLong_AsLong(PyList_GetItem(lst, 3));
    if (PyErr_Occurred() || L < 1 || nr < 1 || nt < 1) {
      PyErr_Clear();
      Py_DECREF(lst);
      continue;
    }
    double fs_hz = PyFloat_AsDouble(PyList_GetItem(lst, 4));
    (void)fs_hz;
    const Py_ssize_t need_body = (Py_ssize_t)nt * nr * L * 2;
    if (n < 5 + need_body) {
      LOG_E(HW, "[SIONNA][RFSIM] FIR list len %zd < 5 + coeffs %zd\n", n, need_body);
      Py_DECREF(lst);
      continue;
    }

    pthread_mutex_lock(&t->sionna_fir_mu);
    if ((size_t)need_body > t->sionna_fir_reim_cap) {
      double *nb = realloc(t->sionna_fir_reim, (size_t)need_body * sizeof(double));
      if (nb) {
        t->sionna_fir_reim = nb;
        t->sionna_fir_reim_cap = (size_t)need_body;
      } else {
        LOG_E(HW, "[SIONNA][RFSIM] realloc FIR buffer failed\n");
        pthread_mutex_unlock(&t->sionna_fir_mu);
        Py_DECREF(lst);
        continue;
      }
    }
    for (Py_ssize_t i = 0; i < need_body; i++) {
      PyObject *it = PyList_GetItem(lst, 5 + i);
      t->sionna_fir_reim[i] = PyFloat_AsDouble(it);
    }
    t->sionna_fir_seq = seq;
    t->sionna_fir_L = L;
    t->sionna_fir_nr = nr;
    t->sionna_fir_nt = nt;
    const double re00 = t->sionna_fir_reim[0];
    const double im00 = (need_body >= 2) ? t->sionna_fir_reim[1] : 0.0;
    const int log_nr = nr;
    const int log_nt = nt;
    const int log_L = L;
    /* tap (tx=1,rx=1,l=0): same linear order as apply_channelmod / Python (tx outer, rx inner) */
    size_t idx_11 = 0;
    double re11 = 0.0, im11 = 0.0;
    if (nr >= 2 && nt >= 2 && log_L >= 1) {
      idx_11 = (size_t)(((1 * nr + 1) * log_L + 0) * 2);
      if (idx_11 + 1 < (size_t)need_body) {
        re11 = t->sionna_fir_reim[idx_11];
        im11 = t->sionna_fir_reim[idx_11 + 1];
      }
    }
    pthread_mutex_unlock(&t->sionna_fir_mu);
    Py_DECREF(lst);

    double scale_log = 1.0;
    const char *sce = getenv("OAI_SIONNA_SCALE");
    if (sce != NULL && sce[0] != '\0') {
      double v = atof(sce);
      if (v > 0.0)
        scale_log = v;
    }
    /* H[0,0]: first tap only for log brevity; full nt*nr*L coeffs are in chanmod (see rfsim_apply_embedded_fir_taps). */
    if ((seq % log_seq_mod) == 0) {
      if (log_nr >= 2 && log_nt >= 2 && log_L >= 1)
        LOG_I(HW,
              "[SIONNA][RFSIM] Updated H seq=%lld (%dx%d), H[0,0]=%f + j%f, H[1,1]=%f + j%f (scale=%f)\n",
              seq,
              log_nr,
              log_nt,
              re00,
        ...
        LOG_I(HW,
              "[SIONNA][RFSIM] Updated H seq=%lld (%dx%d), H[0,0]=%f + j%f (scale=%f)\n",
              seq,
              log_nr,
              log_nt,
              re00,
...
static void maybe_start_sionna_fir_embed_rfsim(rfsimulator_state_t *t)
{
  const char *e = getenv("OAI_SIONNA_FIR_EMBED");
  if (!(e && atoi(e) == 1))
    return;
  if (!t->channelmod) {
    LOG_E(HW, "[SIONNA][RFSIM] FIR embed requires rfsimulator option chanmod\n");
    return;
  }
  ...
  t->sionna_fir_embed = 0;
  t->sionna_fir_seq = -1;
  t->sionna_fir_L = 0;
  t->sionna_fir_nr = 0;
  t->sionna_fir_nt = 0;
  t->sionna_fir_reim = NULL;
  t->sionna_fir_reim_cap = 0;
  t->sionna_fir_running = 1;
  t->sionna_fir_embed = 1;

  /* Python must be initialized on the main thread before any worker uses the C API. */
  if (!Py_IsInitialized()) {
    Py_Initialize();
    PyEval_InitThreads();
    PyEval_SaveThread();
  ...
  if (pthread_create(&t->sionna_fir_thread, NULL, sionna_fir_embed_loop_rfsim, t) != 0) {
    LOG_E(HW, "[SIONNA][RFSIM] FIR embed pthread_create failed\n");
    t->sionna_fir_running = 0;
    t->sionna_fir_embed = 0;
    return;
  }
  LOG_I(HW, "[SIONNA][RFSIM] Enabled: applying Sionna FIR taps to chanmod (H=%dx%d)\n", nrx, ntx);
}

static void maybe_start_sionna_embed_rfsim(rfsimulator_state_t *t)
{
  const char *fir_emb = getenv("OAI_SIONNA_FIR_EMBED");
  if (fir_emb && atoi(fir_emb) == 1)
    return;

  const char *apply_env = getenv("OAI_SIONNA_RFSIM_APPLY");
  if (!(apply_env && atoi(apply_env) == 1))
    return;

  t->sionna_apply = 1;

  const char *scale_env = getenv("OAI_SIONNA_SCALE");
  t->sionna_scale = scale_env ? atof(scale_env) : 1.0;
  if (!(t->sionna_scale > 0.0))
    t->sionna_scale = 1.0;

  const char *diag_env = getenv("OAI_SIONNA_DIAG_ONLY");
  t->sionna_diag_only = diag_env ? atoi(diag_env) : 1; // default ON for safe sync

  const char *shared_file_env = getenv("OAI_SIONNA_SHARED_FILE");
  t->sionna_shared_mode = (shared_file_env && shared_file_env[0] != '\0');
  t->sionna_last_seq = -1;
  t->sionna_shared_file[0] = '\0';
  if (t->sionna_shared_mode) {
    snprintf(t->sionna_shared_file, sizeof(t->sionna_shared_file), "%s", shared_file_env);
  }

  // default to OAI-configured antenna counts
  t->sionna_rx_ant = t->rx_num_channels > 0 ? t->rx_num_channels : 1;
  t->sionna_tx_ant = t->tx_num_channels > 0 ? t->tx_num_channels : 1;
  const char *rx_env = getenv("OAI_SIONNA_RX_ANT");
  const char *tx_env = getenv("OAI_SIONNA_TX_ANT");
  if (rx_env) t->sionna_rx_ant = atoi(rx_env);
  if (tx_env) t->sionna_tx_ant = atoi(tx_env);
  if (t->sionna_rx_ant <= 0) t->sionna_rx_ant = 1;
  if (t->sionna_tx_ant <= 0) t->sionna_tx_ant = 1;

  pthread_mutex_init(&t->sionna_mu, NULL);
  ...
  t->sionna_running = 1;
  if (pthread_create(&t->sionna_thread, NULL, sionna_embed_loop_rfsim, t) != 0) {
    LOG_E(HW, "[SIONNA][RFSIM] pthread_create failed\n");
    t->sionna_running = 0;
    t->sionna_apply = 0;
  } else {
    if (t->sionna_shared_mode) {
      LOG_I(HW,
            "[SIONNA][RFSIM] Enabled(shared): file=%s, applying Sionna H to samples (H=%dx%d)\n",
            t->sionna_shared_file,
            t->sionna_rx_ant,
            t->sionna_tx_ant);
    } else {
      LOG_I(HW, "[SIONNA][RFSIM] Enabled: applying Sionna H to samples (H=%dx%d)\n",
            t->sionna_rx_ant, t->sionna_tx_ant);
    }
  }
}
#endif
...
static void rfsimulator_readconfig(rfsimulator_state_t *rfsimulator) {
  ...
  const char *fir_emb = getenv("OAI_SIONNA_FIR_EMBED");
  if (fir_emb && atoi(fir_emb) == 1) {
    const char *syncf = getenv("OAI_SIONNA_FIR_SYNC_FILE");
    if (syncf != NULL && syncf[0] != '\0') {
      LOG_I(HW,
            "[SIONNA][RFSIM] FIR embed + shared sync file %s (same path on gNB+UE; producer: oai_sionna_fir_taps.py)\n",
            syncf);
    } else {
      LOG_I(HW,
            "[SIONNA][RFSIM] FIR embed per-process Sionna (gNB/UE taps differ unless OAI_SIONNA_FIR_SYNC_FILE is set)\n");
    }
  }
}
...
static int rfsimulator_read(openair0_device *device, openair0_timestamp *ptimestamp, void **samplesVoid, int nsamps, int nbAnt)
{
  ...
  const size_t temp_elems = (size_t)nbAnt * (size_t)nsamps;
  cf_t *temp_block = calloc(temp_elems, sizeof(cf_t));
  AssertFatal(temp_block != NULL,
              "rfsimulator_read: temp_block alloc failed (nbAnt %d nsamps %d)\n",
              nbAnt,
              nsamps);
...
#ifdef OAI_PYTHON_EMBED
      if (ptr->channel_model != NULL && t->sionna_fir_embed) {
        pthread_mutex_lock(&t->sionna_fir_mu);
        if (t->sionna_fir_reim != NULL && t->sionna_fir_L > 0) {
          rfsim_apply_embedded_fir_taps(ptr->channel_model,
                                       t->sionna_fir_seq,
                                       &ptr->sionna_fir_applied_seq,
                                       t->sionna_fir_L,
                                       t->sionna_fir_nr,
                                       t->sionna_fir_nt,
                                       t->sionna_fir_reim);
      ...
      for (int a_rx = 0; a_rx < nbAnt; a_rx++) { // loop over number of Rx antennas
        if (ptr->channel_model != NULL) { // apply a channel model
          if (num_chanmod_channels == 0) {
            memset(temp_block, 0, temp_elems * sizeof(cf_t));
          }
          num_chanmod_channels++;
          rxAddInput(ptr->circularBuf,
                     temp_block + (size_t)a_rx * (size_t)nsamps,
                     a_rx,
                     ptr->channel_model,
                     nsamps,
                     t->nextRxTstamp,
          ...
          if (nbAnt_tx == 1 && t->nb_cnx == 1) { // optimized for 1 Tx and 1 UE
            ...
            for (int i = 0; i < nsamps; i++) { // loop over nsamps
              int32_t acc_r = out[i].r;
              int32_t acc_i = out[i].i;
              for (int a_tx = 0; a_tx < nbAnt_tx; a_tx++) { // sum up signals from nbAnt_tx antennas
                const sample_t x = ptr->circularBuf[((firstIndex + i) * nbAnt_tx + a_tx) % CirSize];

#ifdef OAI_PYTHON_EMBED
                if (t->sionna_apply && t->sionna_h_flat && t->sionna_rx_ant >= nbAnt && t->sionna_tx_ant == nbAnt_tx) {
                  if (t->sionna_diag_only && a_rx != a_tx) {
                    continue;
                  }
                  // H index: (a_rx, a_tx) -> base = 2*(a_rx*nbAnt_tx + a_tx)
                  const int base = 2 * (a_rx * nbAnt_tx + a_tx);
                  double h_re = 0.0, h_im = 0.0;
                  pthread_mutex_lock(&t->sionna_mu);
                  if (base + 1 < t->sionna_h_len) {
                    // normalize by sqrt(Nt) to avoid power blow-up when summing streams
                    const double norm = sqrt((double)nbAnt_tx);
                    h_re = t->sionna_h_flat[base + 0] * (t->sionna_scale / norm);
                    h_im = t->sionna_h_flat[base + 1] * (t->sionna_scale / norm);
                  }
                  pthread_mutex_unlock(&t->sionna_mu);

                  // y += h * x  (complex multiply)
                  acc_r += (int32_t)lround((double)x.r * h_re - (double)x.i * h_im);
                  acc_i += (int32_t)lround((double)x.r * h_im + (double)x.i * h_re);
                } else
#endif
                {
                  // Legacy simple MIMO mixing (real scalar)
              ...
              if (acc_i > 32767) acc_i = 32767;
              if (acc_i < -32768) acc_i = -32768;
  ...
  if (apply_noise_per_channel && num_chanmod_channels > 0) {
    ...
    for (int a = 0; a < nbAnt; a++) {
      ...
      const cf_t *row = temp_block + (size_t)a * (size_t)nsamps;
      for (int i = 0; i < nsamps; i++) {
        out[i].r += lroundf(row[i].r);
        out[i].i += lroundf(row[i].i);
      }
    }
    ...
    for (int a = 0; a < nbAnt; a++) {
      ...
      const cf_t *row = temp_block + (size_t)a * (size_t)nsamps;
      for (int i = 0; i < nsamps; i++) {
        out[i].r += lroundf(row[i].r + noise_power * gaussZiggurat(0.0, 1.0));
        out[i].i += lroundf(row[i].i + noise_power * gaussZiggurat(0.0, 1.0));
      }
    }
  }

  /* AWGN when no chanmod (typical Sionna flat-fading path): global noise_power_dBFS is only set if
   * init_channelmod() ran. Use OAI_RFSIM_AWGN_DBFS so UE/gNB can add AWGN without rfsimulator chanmod. */
  if (num_chanmod_channels == 0) {
    int dbfs = get_noise_power_dBFS();
    const char *awgn_env = getenv("OAI_RFSIM_AWGN_DBFS");
    if (awgn_env != NULL && awgn_env[0] != '\0')
      dbfs = atoi(awgn_env);
    if (dbfs != INVALID_DBFS_VALUE) {
      const int16_t noise_power = (int16_t)(32767.0 / powf(10.0, .05 * (float)(-dbfs)));
      for (int a = 0; a < nbAnt; a++) {
        sample_t *out = (sample_t *)samplesVoid[a];
        for (int i = 0; i < nsamps; i++) {
          int32_t nr = (int32_t)out[i].r + (int32_t)lroundf((float)noise_power * gaussZiggurat(0.0, 1.0));
          int32_t ni = (int32_t)out[i].i + (int32_t)lroundf((float)noise_power * gaussZiggurat(0.0, 1.0));
          if (nr > 32767)
            nr = 32767;
          if (nr < -32768)
            nr = -32768;
...
static void rfsimulator_end(openair0_device *device) {
  ...
  close(s->epollfd);
#ifdef OAI_PYTHON_EMBED
  if (s->sionna_fir_embed) {
    s->sionna_fir_running = 0;
    pthread_join(s->sionna_fir_thread, NULL);
    s->sionna_fir_embed = 0;
    free(s->sionna_fir_reim);
    s->sionna_fir_reim = NULL;
  }
  pthread_mutex_destroy(&s->sionna_fir_mu);
#endif
  free(s);
}
...
#ifdef OAI_PYTHON_EMBED
  pthread_mutex_init(&rfsimulator->sionna_fir_mu, NULL);
#endif
  rfsimulator_readconfig(rfsimulator);
#ifdef OAI_PYTHON_EMBED
  maybe_start_sionna_fir_embed_rfsim(rfsimulator);
  if (!rfsimulator->sionna_fir_embed)
    maybe_start_sionna_embed_rfsim(rfsimulator);
#endif
