...
/** OAI_RFSIM_CONV_LOG=1: log discrete convolution in rxAddInput (first blocks only; see env below). */
static void rfsim_conv_log_read_cfg(int *enabled, int *nsamp, int *max_blocks, int *max_taps)
{
  static int init = 0;
  static int e, n, m, t;
  if (!init) {
    init = 1;
    const char *ev = getenv("OAI_RFSIM_CONV_LOG");
    e = (ev != NULL && (ev[0] == '1' || ev[0] == 'y' || ev[0] == 'Y'));
    n = 32;
    const char *ns = getenv("OAI_RFSIM_CONV_LOG_NSAMP");
    if (ns != NULL && ns[0] != '\0')
      n = atoi(ns);
    if (n < 1)
      n = 1;
    if (n > 4096)
      n = 4096;
    m = 1;
    const char *mb = getenv("OAI_RFSIM_CONV_LOG_BLOCKS");
    if (mb != NULL && mb[0] != '\0')
      m = atoi(mb);
    if (m < 1)
      m = 1;
    if (m > 100000)
      m = 100000;
    t = 16;
    const char *mt = getenv("OAI_RFSIM_CONV_LOG_TAPS");
    if (mt != NULL && mt[0] != '\0')
      t = atoi(mt);
    if (t < 1)
      t = 1;
                ...
                bool add_noise)
{
  ...
  if (rxAnt < 0 || rxAnt >= channelDesc->nb_rx) {
    LOG_E(HW, "rxAddInput: rxAnt %d out of range (nb_rx %d)\n", rxAnt, channelDesc->nb_rx);
    return;
  }
  double Doppler_phase_cur = 0.0;
  if (channelDesc->Doppler_phase_cur != NULL) {
    Doppler_phase_cur = (double)channelDesc->Doppler_phase_cur[rxAnt];
    Doppler_phase_cur -= 2 * M_PI * round(Doppler_phase_cur / (2 * M_PI));
  }

  static uint64_t rfsim_conv_log_block_count = 0;
  int conv_en = 0, conv_nsamp = 0, conv_max_blocks = 0, conv_max_taps = 0;
  rfsim_conv_log_read_cfg(&conv_en, &conv_nsamp, &conv_max_blocks, &conv_max_taps);
  const bool conv_log_this =
      (conv_en != 0 && rfsim_conv_log_block_count < (uint64_t)conv_max_blocks);
  bool conv_logged_nonzero_sample = false;
  int conv_hits_logged = 0;
  const int conv_hits_max = 64;

  if (conv_log_this) {
    LOG_I(HW,
          "[rfsim_conv] rx=%d TS=%llu nbSamples=%d Cir=%u dd=%llu L=%d nbTx=%d nbRx=%d path_loss_dB=%f\n",
          rxAnt,
    ...
    int Ldump = (int)channelDesc->channel_length;
    if (Ldump > conv_max_taps)
      Ldump = conv_max_taps;
    for (int txAnt = 0; txAnt < nbTx && txAnt < 2; txAnt++) {
      const struct complexd *cm = channelDesc->ch[rxAnt + (txAnt * channelDesc->nb_rx)];
      for (int l = 0; l < Ldump; l++)
        LOG_I(HW, "[rfsim_conv] h[rx=%d tx=%d][%d] = %g + j%g\n", rxAnt, txAnt, l, cm[l].r, cm[l].i);
    }
  }
  ...
  for (int i=0; i<nbSamples; i++) {
    ...
    for (int txAnt=0; txAnt < nbTx; txAnt++) {
      ...
      for (int l = 0; l<(int)channelDesc->channel_length; l++) {
        ...
        /* Signed sample index: uint64_t (TS - dd + i - l) underflows when TS+i < l+dd and corrupts idx. */
        int64_t sbase = (int64_t)TS - (int64_t)dd + (int64_t)i - (int64_t)l;
        if (sbase < 0)
          continue;
        int64_t spos = sbase * (int64_t)nbTx + (int64_t)txAnt;
        int64_t mod = (int64_t)CirSize;
        int64_t idx64 = ((spos % mod) + mod) % mod;
        const int idx = (int)idx64;
        const struct complex16 tx16 = input_sig[idx];
        const double contrib_r = tx16.r * channelModel[l].r - tx16.i * channelModel[l].i;
        const double contrib_i = tx16.i * channelModel[l].r + tx16.r * channelModel[l].i;
        if (conv_log_this && i < conv_nsamp) {
          const bool nonzero_contrib = (fabs(contrib_r) > 1e-12 || fabs(contrib_i) > 1e-12);
          if (nonzero_contrib && conv_hits_logged < conv_hits_max) {
            conv_logged_nonzero_sample = true;
            conv_hits_logged++;
            LOG_I(HW,
                  "[rfsim_conv_hit] i=%d tx=%d l=%d idx=%d x=(%d,%d) h=(%g,%g) contrib=(%g,%g)\n",
                  i,
                  txAnt,
                  l,
                  idx,
    ...
    if (conv_log_this && i < conv_nsamp) {
      const bool nonzero_y = (fabs(rx_tmp.r) > 1e-12 || fabs(rx_tmp.i) > 1e-12);
      if (nonzero_y) {
        conv_logged_nonzero_sample = true;
        LOG_I(HW, "[rfsim_conv_sum] i=%d y_prePL=(%g,%g)\n", i, rx_tmp.r, rx_tmp.i);
      }
    }
  ...
  if (conv_log_this) {
    if (!conv_logged_nonzero_sample)
      LOG_I(HW, "[rfsim_conv] all sampled i in this block were zero (x and y_prePL)\n");
    else
      LOG_I(HW, "[rfsim_conv] logged nonzero sample activity (hits=%d)\n", conv_hits_logged);
    rfsim_conv_log_block_count++;
  }

  if (channelDesc->Doppler_phase_cur != NULL)
    channelDesc->Doppler_phase_cur[rxAnt] = (float)Doppler_phase_cur;
...
int rfsim_apply_embedded_fir_taps(channel_desc_t *cd,
                                  long long seq,
                                  long long *per_buf_applied_seq,
                                  int L,
                                  int nr,
                                  int nt,
                                  const double *reim)
{
  if (cd == NULL || per_buf_applied_seq == NULL || reim == NULL)
    return -2;
  if (seq < 0 || L < 1 || L > 4096 || nr < 1 || nt < 1)
    return -2;
  if (seq == *per_buf_applied_seq)
    return -1;
  if (nr != (int)cd->nb_rx || nt != (int)cd->nb_tx) {
    LOG_E(HW,
          "[FIR_EMBED] (nr=%d nt=%d) != channel_desc (nb_rx=%u nb_tx=%u)\n",
          nr,
          nt,
          cd->nb_rx,
          cd->nb_tx);
  ...
  const int chlen = (int)cd->channel_length;
  const int Lclip = (L < chlen) ? L : chlen;
  if (L > chlen)
    LOG_W(HW, "[FIR_EMBED] L=%d > channel_length=%d; clipping\n", L, chlen);

  const int nlinks = (int)cd->nb_rx * (int)cd->nb_tx;
  size_t idx = 0;
  for (int tx = 0; tx < nt; tx++) {
    for (int rx = 0; rx < nr; rx++) {
      const int k = rx + tx * nr;
      if (k < 0 || k >= nlinks || cd->ch[k] == NULL) {
        LOG_E(HW, "[FIR_EMBED] bad link index k=%d\n", k);
        return -2;
      }
      for (int l = 0; l < L; l++) {
        double re = reim[idx++];
