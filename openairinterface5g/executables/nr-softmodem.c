...
#ifdef OAI_PYTHON_EMBED
// ---- Sionna Python embedding POC (periodic channel coefficient update) ----
static pthread_t sionna_thread;
static pthread_mutex_t sionna_mu = PTHREAD_MUTEX_INITIALIZER;
static double *sionna_h_flat = NULL; // [re00, im00, re01, im01, ...] row-major
static int sionna_rx_ant = 1;
static int sionna_tx_ant = 1;
static int sionna_h_len = 0; // number of doubles in sionna_h_flat
static volatile int sionna_running = 0;

typedef struct {
  char sionna_path[4096];
} sionna_embed_args_t;

static void *sionna_embed_loop(void *arg)
{
  sionna_embed_args_t *a = (sionna_embed_args_t *)arg;

  // Each thread must hold the GIL when calling into Python
  PyGILState_STATE gstate = PyGILState_Ensure();

  // Add sionna-main to sys.path
  char cmd[8192];
  snprintf(cmd,
           sizeof(cmd),
           "import sys\n"
  ...
  if (!mod) {
    PyErr_Print();
    LOG_E(HW, "[SIONNA][EMBED] Failed to import oai_channel_embed\n");
  ...
  if (init_fn && PyCallable_Check(init_fn)) {
    ...
    PyDict_SetItemString(kwargs, "num_rx_ant", PyLong_FromLong(sionna_rx_ant));
    PyDict_SetItemString(kwargs, "num_tx_ant", PyLong_FromLong(sionna_tx_ant));
    PyObject *ret = PyObject_Call(init_fn, PyTuple_New(0), kwargs);
    Py_DECREF(kwargs);
    if (!ret) {
      PyErr_Print();
      LOG_E(HW, "[SIONNA][EMBED] init() call failed\n");
    } else {
      Py_DECREF(ret);
    }
  }
  ...
  if (!get_fn || !PyCallable_Check(get_fn)) {
    LOG_E(HW, "[SIONNA][EMBED] get_h_flat not callable\n");
    Py_XDECREF(get_fn);
    Py_DECREF(mod);
    PyGILState_Release(gstate);
    free(a);
  ...
  while (sionna_running) {
    ...
    pthread_mutex_lock(&sionna_mu);
    if (sionna_h_len != (int)n) {
      free(sionna_h_flat);
      sionna_h_flat = calloc((size_t)n, sizeof(double));
      sionna_h_len = (int)n;
    }
    for (Py_ssize_t i = 0; i < n; i++) {
      PyObject *item = PyList_GetItem(lst, i); // borrowed
      if (item)
        sionna_h_flat[i] = PyFloat_AsDouble(item);
    }
    pthread_mutex_unlock(&sionna_mu);

    Py_DECREF(lst);

    if ((iter++ % 10) == 0) { // ~1초에 1번 (100ms 주기 기준)
      // log first element h00
      double re00 = 0.0, im00 = 0.0;
      pthread_mutex_lock(&sionna_mu);
      if (sionna_h_flat && sionna_h_len >= 2) {
        re00 = sionna_h_flat[0];
        im00 = sionna_h_flat[1];
      }
      pthread_mutex_unlock(&sionna_mu);
      LOG_I(HW, "[SIONNA][EMBED] Updated H (%dx%d), H[0,0]=%f + j%f\n", sionna_rx_ant, sionna_tx_ant, re00, im00);
    }
...
static void maybe_start_sionna_embed_thread(void)
{
  const char *embed_env = getenv("OAI_SIONNA_EMBED");
  if (!(embed_env && atoi(embed_env) == 1))
    return;

  const char *rx_env = getenv("OAI_SIONNA_RX_ANT");
  const char *tx_env = getenv("OAI_SIONNA_TX_ANT");
  if (rx_env)
    sionna_rx_ant = atoi(rx_env);
  if (tx_env)
    sionna_tx_ant = atoi(tx_env);
  if (sionna_rx_ant <= 0) sionna_rx_ant = 1;
  if (sionna_tx_ant <= 0) sionna_tx_ant = 1;

  // Initialize Python once in main thread
  Py_Initialize();
  PyEval_InitThreads(); // deprecated but still required on some builds for embedding init
  PyEval_SaveThread();  // release GIL for other threads

  sionna_embed_args_t *a = calloc(1, sizeof(*a));
  if (!a) {
    LOG_E(HW, "[SIONNA][EMBED] calloc failed\n");
    return;
  }

  // assume cwd is project root; build absolute path to sionna-main
  char cwd[4096] = {0};
  if (getcwd(cwd, sizeof(cwd) - 1) != NULL)
    snprintf(a->sionna_path, sizeof(a->sionna_path), "%s/sionna-main", cwd);
  else
    snprintf(a->sionna_path, sizeof(a->sionna_path), "./sionna-main");

  sionna_running = 1;
  if (pthread_create(&sionna_thread, NULL, sionna_embed_loop, a) != 0) {
    LOG_E(HW, "[SIONNA][EMBED] pthread_create failed\n");
    sionna_running = 0;
    free(a);
  } else {
    LOG_I(HW, "[SIONNA][EMBED] Enabled (sys.path += %s)\n", a->sionna_path);
  }
}
#endif
...
int main( int argc, char **argv ) {
  ...
  maybe_start_sionna_embed_thread();
#else
  // Legacy POC: 외부 Python 스크립트 + 파일 I/O
  // 스크립트 경로: ./sionna-main/oai_channel_poc.py
  // 출력 파일:     ./channel_coeff.txt
  {
    int py_ret = system("cd ./sionna-main && python3 oai_channel_poc.py");
    if (py_ret != 0) {
      LOG_W(HW, "[POC] Python 채널 POC 스크립트 실행 실패 (ret=%d)\n", py_ret);
    } else {
      FILE *cf = fopen("channel_coeff.txt", "r");
      if (cf == NULL) {
        LOG_W(HW, "[POC] channel_coeff.txt를 열 수 없습니다\n");
      } else {
        double h_re = 0.0, h_im = 0.0;
        int nread = fscanf(cf, "%lf %lf", &h_re, &h_im);
        fclose(cf);
        if (nread == 2) {
          LOG_I(HW, "[POC] Python에서 읽은 채널 계수 h = %f + j%f\n", h_re, h_im);
        } else {
          LOG_W(HW, "[POC] channel_coeff.txt에서 채널 계수 2개를 읽지 못했습니다 (nread=%d)\n", nread);
        }
      }
    }
  }
