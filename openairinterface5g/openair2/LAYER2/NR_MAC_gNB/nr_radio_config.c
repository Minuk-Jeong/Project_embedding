...
static void set_dl_DataToUL_ACK(NR_PUCCH_Config_t *pucch_Config, int min_feedback_time, NR_SubcarrierSpacing_t subcarrierSpacing)
{
  ...
  // ASN.1 constraint: SEQUENCE (SIZE (1..8)) OF INTEGER (0..15)
  // min_feedback_time > 8 can make (min_feedback_time + i) exceed 15 and break encoding.
  int start_delay = min_feedback_time;
  if (start_delay < 0)
    start_delay = 0;
  if (start_delay > 15) {
    LOG_W(NR_RRC,
          "min_feedback_time %d (SCS %ld) exceeds dl-DataToUL-ACK range, clamping to 15\n",
          min_feedback_time,
  ...
  // Keep one valid value even in pathological configurations.
  if (pucch_Config->dl_DataToUL_ACK->list.count == 0) {
    long *fallback = calloc(1, sizeof(*fallback));
    *fallback = 15;
    asn1cSeqAdd(&pucch_Config->dl_DataToUL_ACK->list, fallback);
  }
}
