...
// Ported from Project_debugging5/openairinterface5g (legacy path only; no ONNX dependency)
                                       ...
                                       uint8_t *rank_indicator)
{
  ...
  *rank_indicator = 0;
  ...
  // For each CSI-RS RB/subcarrier compute H^H*H (4x4) and evaluate condition number from 2x2 submatrices
  for (int rb = csirs_config_pdu->start_rb; rb < (csirs_config_pdu->start_rb + csirs_config_pdu->nr_of_rbs); rb++) {
    if (csirs_config_pdu->freq_density <= 1 && csirs_config_pdu->freq_density != (rb % 2)) {
      continue;
    }
    ...
    for (int sc_idx = 0; sc_idx < NR_NB_SC_PER_RB; sc_idx++) {
      ...
      for (int port_tx_i = 0; port_tx_i < 4; port_tx_i++) {
        for (int port_tx_j = 0; port_tx_j < 4; port_tx_j++) {
          ...
          for (int ant_rx = 0; ant_rx < 4; ant_rx++) {
            ...
            const c16_t h = csi_rs_estimated_channel_freq[ant_rx][port_tx_j][sc_k_offset];

            // prod = conj(h_i) * h_j  (scaled down by log2_maxh like existing 2x2 path)
            c16_t prod;
      ...
      // Evaluate 2x2 submatrices: (0,1)x(0,1) and (2,3)x(2,3), average their cond number (dB)
      int32_t avg_cond_num = 0;
      int num_submatrices = 0;
      for (int base_i = 0; base_i < 3; base_i += 2) {
        const int i = base_i;
        const int j = base_i + 1;

        const c16_t a_ii = A_MF[i][i];
        const c16_t a_jj = A_MF[j][j];
        const c16_t a_ij = A_MF[i][j];

        // det ~= real(a_ii*a_jj) - |a_ij|^2  (A_MF is Hermitian, so this is valid approximation)
        const int32_t a_ii_a_jj_re = ((int32_t)a_ii.r * (int32_t)a_jj.r) - ((int32_t)a_ii.i * (int32_t)a_jj.i);
        const int32_t a_ij_abs2 = ((int32_t)a_ij.r * (int32_t)a_ij.r) + ((int32_t)a_ij.i * (int32_t)a_ij.i);
        const int32_t det = abs(a_ii_a_jj_re - a_ij_abs2);

        const int32_t numer = ((int32_t)a_ii.r * (int32_t)a_ii.r) + ((int32_t)a_ii.i * (int32_t)a_ii.i) +
                              ((int32_t)a_jj.r * (int32_t)a_jj.r) + ((int32_t)a_jj.i * (int32_t)a_jj.i);

        if (det > 0) {
          const int32_t numer_db = dB_fixed(numer);
          const int32_t det_db = dB_fixed(det);
          avg_cond_num += (numer_db - det_db);
          num_submatrices++;
        }
      }

      if (num_submatrices > 0) {
        avg_cond_num /= num_submatrices;
  ...
  // If condition number is low on half+ of REs -> rank > 1
  if (count > 0)
    *rank_indicator = 1;
  else
    *rank_indicator = 0;
...
static inline void matmul_H_W(const c16_t H[4][4], int Nr, int Nt, const c16_t W[4][2], int L, int32_t R_re[4][2], int32_t R_im[4][2])
{
  for (int r = 0; r < Nr; r++) {
    for (int l = 0; l < L; l++) {
      int64_t acc_re = 0, acc_im = 0;
      for (int t = 0; t < Nt; t++) {
        const int16_t hr = H[r][t].r, hi = H[r][t].i;
        const int16_t wr = W[t][l].r, wi = W[t][l].i;
...
static inline uint64_t mmse_sinr_metric_L1_L2(const c16_t H[4][4], int Nr, int Nt, const c16_t W[4][2], int L, uint32_t nVar, uint64_t out_gamma[2])
{
  int32_t R_re[4][2] = {{0}};
  int32_t R_im[4][2] = {{0}};
  ...
  if (L == 1) {
    ...
    if (nVar > 0) {
      const uint64_t s_q50 = s << 20;
      if (s_q50 >= (uint64_t)nVar)
  ...
  uint64_t a = (uint64_t)nVar;
  uint64_t d = (uint64_t)nVar;
  int64_t b_re = 0, b_im = 0;
  for (int r = 0; r < Nr; r++) {
    const int32_t r0_re = R_re[r][0], r0_im = R_im[r][0];
    const int32_t r1_re = R_re[r][1], r1_im = R_im[r][1];
    a += (uint64_t)cabs2_i32(r0_re, r0_im);
    d += (uint64_t)cabs2_i32(r1_re, r1_im);
    b_re += (int64_t)r0_re * r1_re + (int64_t)r0_im * r1_im;
    b_im += -(int64_t)r0_im * r1_re + (int64_t)r0_re * r1_im;
  }

  __int128 ad = (__int128)a * (__int128)d;
  __int128 bb = (__int128)b_re * (__int128)b_re + (__int128)b_im * (__int128)b_im;
  __int128 det = ad - bb;
  if (det <= 0 || nVar == 0) {
    out_gamma[0] = 0;
    out_gamma[1] = 0;
  ...
  __int128 denom0 = (__int128)nVar * (__int128)d;
  __int128 denom1 = (__int128)nVar * (__int128)a;
  __int128 t0_q20 = (det << 20) / denom0;
  __int128 t1_q20 = (det << 20) / denom1;
  __int128 one_q20 = (__int128)1 << 20;
  uint64_t gamma0_q20 = 0, gamma1_q20 = 0;
  if (t0_q20 > one_q20) {
    gamma0_q20 = (uint64_t)(t0_q20 - one_q20);
    if (gamma0_q20 > UINT32_MAX) gamma0_q20 = UINT32_MAX;
  }
  if (t1_q20 > one_q20) {
    gamma1_q20 = (uint64_t)(t1_q20 - one_q20);
    if (gamma1_q20 > UINT32_MAX) gamma1_q20 = UINT32_MAX;
  }
...
static inline void build_codebook_4port_rank1(int i11, int i2, c16_t W[4][2]) {
  ...
  c16_t ejt = e8[i11 & 7];
  c16_t pol = q15_from_phase_quarter(i2);
  memset(W, 0, sizeof(c16_t) * 4 * 2);
  W[0][0] = (c16_t){a, 0};
  W[1][0] = (c16_t){ (int16_t)(((int32_t)a * ejt.r) >> 15), (int16_t)(((int32_t)a * ejt.i) >> 15) };
  W[2][0] = (c16_t){ (int16_t)(((int32_t)a * pol.r) >> 15), (int16_t)(((int32_t)a * pol.i) >> 15) };
  int32_t re = cmul_re_q15(pol.r, pol.i, ejt.r, ejt.i);
  int32_t im = cmul_im_q15(pol.r, pol.i, ejt.r, ejt.i);
  W[3][0] = (c16_t){ (int16_t)(((int32_t)a * re) >> 15), (int16_t)(((int32_t)a * im) >> 15) };
}
...
static inline void build_codebook_4port_rank2(int i11, int i13, int i2, c16_t W[4][2]) {
  ...
  c16_t ejt = e8[i11 & 7];
  c16_t pol0 = q15_from_phase_quarter(i2);
  c16_t pol1 = q15_from_phase_quarter(i2 + 2 * (i13 & 1));
  memset(W, 0, sizeof(c16_t) * 4 * 2);

  W[0][0] = (c16_t){a,0};
  W[1][0] = (c16_t){ (int16_t)(((int32_t)a * ejt.r) >> 15), (int16_t)(((int32_t)a * ejt.i) >> 15) };
  W[2][0] = (c16_t){ (int16_t)(((int32_t)a * pol0.r) >> 15), (int16_t)(((int32_t)a * pol0.i) >> 15) };
  int32_t re0 = cmul_re_q15(pol0.r, pol0.i, ejt.r, ejt.i);
  int32_t im0 = cmul_im_q15(pol0.r, pol0.i, ejt.r, ejt.i);
  W[3][0] = (c16_t){ (int16_t)(((int32_t)a * re0) >> 15), (int16_t)(((int32_t)a * im0) >> 15) };

  W[0][1] = (c16_t){a,0};
  W[1][1] = (c16_t){ (int16_t)(-(((int32_t)a * ejt.r) >> 15)), (int16_t)(-(((int32_t)a * ejt.i) >> 15)) };
  W[2][1] = (c16_t){ (int16_t)(((int32_t)a * pol1.r) >> 15), (int16_t)(((int32_t)a * pol1.i) >> 15) };
  ...
  W[3][1] = (c16_t){ (int16_t)(((int32_t)a * re1) >> 15), (int16_t)(((int32_t)a * im1) >> 15) };
}
                             ...
                             uint32_t *precoded_sinr_dB)
{
  ...
  if (N_ports == 4 && (rank_indicator == 0 || rank_indicator == 1)) {
    const int Nr = frame_parms->nb_antennas_rx;
    const int Nt = 4;
    const int L = (rank_indicator == 0) ? 1 : 2;
    const int cand_count = (L == 1) ? 16 : 32;
    uint64_t cand_metric[32] = {0};
    int cand_n[32] = {0};
    ...
    for (int rb = csirs_config_pdu->start_rb; rb < (csirs_config_pdu->start_rb + csirs_config_pdu->nr_of_rbs); rb++) {
      ...
      for (int ant_rx = 0; ant_rx < Nr; ant_rx++) {
        for (int port = 0; port < Nt; port++) {
          H[ant_rx][port] = csi_rs_estimated_channel_freq[ant_rx][port][k_offset];
        }
      }

      if (L == 1) {
        for (int i11 = 0; i11 < 8; i11++) {
          for (int cand_i2 = 0; cand_i2 < 2; cand_i2++) {
            const int idx = i11 * 2 + cand_i2;
            c16_t W[4][2];
            uint64_t gamma[2] = {0};
            build_codebook_4port_rank1(i11, cand_i2, W);
            cand_metric[idx] += mmse_sinr_metric_L1_L2(H, Nr, Nt, W, L, interference_plus_noise_power, gamma);
            cand_n[idx]++;
          }
        }
      } else {
        for (int i11 = 0; i11 < 8; i11++) {
          for (int i13 = 0; i13 < 2; i13++) {
            for (int cand_i2 = 0; cand_i2 < 2; cand_i2++) {
              const int idx = i11 * 4 + i13 * 2 + cand_i2;
              c16_t W[4][2];
              uint64_t gamma[2] = {0};
              build_codebook_4port_rank2(i11, i13, cand_i2, W);
              cand_metric[idx] += mmse_sinr_metric_L1_L2(H, Nr, Nt, W, L, interference_plus_noise_power, gamma);
              cand_n[idx]++;
            }
          }
    ...
    for (int c = 0; c < cand_count; c++) {
      if (cand_n[c] == 0) continue;
      uint64_t avg = cand_metric[c] / (uint64_t)cand_n[c];
      if (avg > best_avg) {
        best_avg = avg;
        best = c;
      }
    ...
    uint32_t sinr_linear = (uint32_t)(best_avg >> 20);
    if (sinr_linear == 0) sinr_linear = 1;
    *precoded_sinr_dB = dB_fixed(sinr_linear);
  }
  else if(rank_indicator == 0 || rank_indicator == 1) {
    c64_t sum[4] = {0};
    c64_t sum2[4] = {0};
