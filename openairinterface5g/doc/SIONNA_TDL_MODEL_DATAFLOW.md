# Sionna TR38.901 TDL 모델: 채널 생성 데이터 플로우

## 대상 코드
- `sionna-main/src/sionna/phy/channel/tr38901/tdl.py`

## 목적
이 문서는 Sionna의 `TDL`(TR38.901 tapped delay line) 채널 모델이
`TDL.__init__()`과 `TDL.__call__()`에서 어떤 난수/연산을 거쳐
복소 채널 계수(`h`)와 경로 지연(`tau`)을 생성하는지, 코드 근거를 그대로 포함해 정리합니다.

## 데이터 흐름 요약

1. `__init__()`
   - 모델(A~E 등)에 대응하는 `TDL-*.json`에서 PDP(지연/파워)와 LoS 여부를 읽고 정규화합니다.
   - `min_speed`/`max_speed`로부터 도플러 범위를 미리 계산합니다.
   - (옵션) 공간 상관행렬의 Cholesky 같은 분해를 미리 계산합니다.

2. `__call__(batch_size, num_time_steps, sampling_frequency)`
   - time axis(`sample_times`)를 만듭니다.
   - 각 호출마다 도플러(`doppler`), 도래각 계열(`theta`), 위상(`phi`)을 무작위로 생성합니다.
   - SoS(sum-of-sinusoids) 형태로 경로별 복소 계수 `h`를 만들고, PDP 평균 파워로 스케일합니다.
   - LoS면 첫 탭을 rician 성분으로 보정합니다.
   - 경로 지연(`delays`)을 초 단위로 만들고, 필요시 공간 상관을 적용합니다.
   - 최종으로 `h`와 `delays`를 반환합니다.

---

## 근거 코드 (`tdl.py` 전체)
아래 코드는 설명의 근거로 그대로 포함합니다.
